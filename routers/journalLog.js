...
router.patch('/logs/:id', auth, async (req, res) => {
    try{
        const log = await JournalLog.findOne({ _id: req.params.id, owner: req.user._id })
...
router.delete('/logs/:id', auth, async (req, res) => {
    try {
        const journalLog = await JournalLog.findOneAndDelete({ _id: req.params.id, owner: req.user._id })
